...
// not bind
$query = sprintf("select `name`, `assistant` from `promogirls_userdata` where `status` = 0 and `name` not in ('GGK', 'MG', '小花', '杨育琪', '黎红红', '谢雁莎', '何雪影', '陈秋燕', '赖翠玲', '蔡碧娟', '何飞梅', '黄静芹', '石海', 'Ben', '梁翠莲', '王燕萍', '张吕', '李叶', '郝磊', '戴岱', '郑银莲');");
...
$query = sprintf("select `id`, `name`, `assistant`, `point__all`, `point__used` from `promogirls_users` where `status` = 1 and `name` not in ('GGK', 'MG', '小花', '杨育琪', '黎红红', '谢雁莎', '何雪影', '陈秋燕', '赖翠玲', '蔡碧娟', '何飞梅', '黄静芹', '石海', 'Ben', '梁翠莲', '王燕萍', '张吕', '李叶', '郝磊', '戴岱', '郑银莲');");
$source = $PG->query($query);
if($source->num_rows > 0) {
    while($row = $source->fetch_assoc()) {
        $bind_yes[] = $row;
...
foreach($bind_yes as $k=>$bindyes) {
    $query = sprintf("select `count`, `dateline_expire` from `promogirls_qrcodes_scan_log` where `uid` = %d order by `dateline_expire` asc;", 
        $bindyes['id']
    );
    $source = $PG->query($query);
    if($source->num_rows > 0) {
...
$begin      = new DateTime(date('Y-m-d', strtotime("-1 week +1 day")));
...
foreach($bind_yes as $bind) {
    ...
    echo '<td>'.max(0, $bind['point__all']).'</td>';
    echo '<td>'.max(0, $bind['point__used']).'</td>';
    echo '<td>'.max(0, ($bind['point__all'] - $bind['point__used'])).'</td>';
    echo "</tr>";
}
...
$query = "select DATE_FORMAT(`dateline`, '%H') as `key`, DATE_FORMAT(`dateline`, '%Y-%m-%d') as `day`, DATE_FORMAT(`dateline`, '%Y-%m-%d %H') as `hour`, count(1) as `count` from `letsktv_logs_subscribe` group by `hour` order by `key` asc, `day` asc;";
$source = $WX->query($query);
echo '时段新增数<br />';
echo '<table>'."\n";
if($source->num_rows > 0) {
    while($row = $source->fetch_assoc()) {
        $byhours[$row['key']][$row['day']] = $row['count'];
    }
}
...
for($i = 0; $i < 24; $i++) {
    ...
    echo '<tr><td>'.$j.'</td>';
    foreach($daterange as $date){
        if(isset($byhours[$j]) && isset($byhours[$j][$date->format("Y-m-d")])) {
            echo '<td>'.$byhours[$j][$date->format("Y-m-d")].'</td>';
        } else {
            echo '<td>0</td>';
        }
    }
