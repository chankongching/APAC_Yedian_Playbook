...
foreach($bind_yes as $k=>$bindyes) {
    $query = sprintf("select `count`, `dateline_expire` from `promogirls_qrcodes_scan_log` where `uid` = %d and `dateline_expire` between '2015-12-14 01:00:00' and '2015-12-21 00:59:59' order by `dateline_expire` asc;", 
        $bindyes['id']
    );
    $source = $PG->query($query);
    if($source->num_rows > 0) {
...
$begin      = new DateTime('2015-12-14');
$end        = new DateTime('2015-12-21');
