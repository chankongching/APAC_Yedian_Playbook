$(document).ready(function(){
	...
	//Try 2nd time to highlight current / active link
        if(is_highlighted == false) {
            $('ul.main-menu li a').each(function(){
                    link_url = $($(this))[0].href;
                    cur_url = String(window.location);
                    link_array = link_url.split('?');
                    cur_array = cur_url.split('?');
                    ...
                    if(link_array.join('') == cur_array.join('')) {
                        $(this).parent().addClass('active');
                        is_highlighted = true;
                    }
            });
	...
	//Try 3rd time to highlight current / active link
        if(is_highlighted == false) {
            $('ul.main-menu li a').each(function(){
                    link_url = $($(this))[0].href;
                    cur_url = String(window.location);
                    link_array = link_url.split('/');
                    cur_array = cur_url.split('/');
                    while(cur_array.length > link_array.length) {
                        cur_array.pop();
                    }
                    if(link_array.join('/') == cur_array.join('/')) {
                        $(this).parent().addClass('active');
	...
	//Try last time to highlight current / active link
        if(is_highlighted == false) {
            $('ul.main-menu li a').each(function(){
                    link_url = $($(this))[0].href;
                    cur_url = String(window.location);
                    ...
                    if(cur_array.length > 0 && link_array.join('/') == cur_array.join('/')) {
                        $(this).parent().addClass('active');
                    }
            });
        }
	...
	$('a.loading-link').click(function(e){
		//e.preventDefault();
		$('#loading').remove();
		$('#content').fadeOut().parent().append('<div id="loading" class="center">Loading...<div class="center"></div></div>');
                return;
...
function docReady(){
	...
	//category controlls container animation
	...
	//gallery colorbox
        if($('.gallery a').length > 0) {
            ...
            $('.gallery a').colorbox({rel:'gallery a', transition:"elastic", maxWidth:"95%", maxHeight:"95%"});
        }
