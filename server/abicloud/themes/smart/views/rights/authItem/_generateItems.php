	...
	<?php foreach( $items['controllers'] as $key=>$item ): ?>
		...
		<?php if( isset($item['actions'])===true && $item['actions']!==array() ): ?>
			...
			<tr class="controller-row <?php echo $controllerExists===true ? 'exists' : ''; ?>">
				<td class="checkbox-column" style='background-color:lightblue;'><?php echo $controllerExists===false ? $form->checkBox($model, 'items['.$controllerKey.'.*]') : ''; ?></td>
				<td class="name-column" style='background-color:lightblue;'><?php echo ucfirst($item['name']).'.*'; ?></td>
				<td class="path-column" style='background-color:lightblue;'><?php echo substr($item['path'], $basePathLength+1); ?></td>
			</tr>
