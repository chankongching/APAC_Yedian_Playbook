...
class QRCodeGenerator extends CWidget {
    ...
    public function init() {
        ...
        if (is_null($this->filename)) {
            $this->filename = 'QR_' . md5($this->data) . '.png';
        }

        if (!$this->filePath) {
            $this->filePath = (empty(Yii::app()->params['qrcode_folder']) ? (Yii::getPathOfAlias('webroot.uploads') . DIRECTORY_SEPARATOR . 'qr') : Yii::app()->params['qrcode_folder']);
        ...
        if (!isset($this->fileUrl)) {
            $this->fileUrl = (empty(Yii::app()->params['qrcode_url']) ? Yii::app()->baseUrl . '/uploads/qr' : Yii::app()->params['qrcode_url']);
        ...
        if (is_string($this->subfolderVar)) {
            $subfolder = $this->filePath . DIRECTORY_SEPARATOR . $this->subfolderVar;
            if (!is_dir($subfolder)) {
                mkdir($subfolder);
                chmod($subfolder, 0777);
            }
            $this->filePath = $this->filePath . DIRECTORY_SEPARATOR . $this->subfolderVar;
    ...
    public function run() {
        ...
        if ($this->checkExists && is_file($this->filePath)) {
            Yii::trace('QRcode ' . $this->filePath . ' exists, not re-created.');
        } else {
            $code = new QRCode($this->data);
            $code->create($this->filePath);
        }
