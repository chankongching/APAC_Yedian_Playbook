...
class CURL extends CComponent {
    ...
    function __construct($url = '') {
        ...
        if (!$this->is_enabled()) {
            self::log('cURL Class - PHP was not built with cURL enabled. Rebuild PHP with --with-curl to use cURL.');
        }
    ...
    public function post_with_file($params = array(), $options = array()) {
        // If its an array (instead of a query string) then format it correctly
        //if (is_array($params)) {
        //    $params = http_build_query($params, NULL, '&');
        //}
    ...
    public function execute() {
        ...
        $this->options();
        //$this->log(print_r($this->options,true), 'trace');
        ...
        // Request successful
            ...
            $this->request = curl_getinfo($this->session, CURLINFO_HEADER_OUT);
            curl_close($this->session);
