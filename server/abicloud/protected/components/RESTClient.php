...
class RESTClient extends CComponent {
    ...
    public function get($uri, $params = array(), $format = NULL) {
        if ($params) {
            ...
            $this->log("HTTP GET Parameters:\n" . print_r($params, true), 'trace');
        }
    ...
    public function post($uri, $params = array(), $format = NULL) {
        ...
        $response = $this->_call('post', $uri, $params, $format);
        $request_header = $this->_curl->get_request_header();
        $this->log("HTTP POST Header:\n" . print_r($request_header, true), 'trace');
        $this->log("HTTP POST Parameters:\n" . print_r($params, true), 'trace');
        return $response;
        //return $this->_call('post', $uri, $params, $format);
    }

    /**
     * Perform CURL POST request with file
     * @param string $uri	: Relative URI resource on server.
     * @param array $params : Parameters to send as POST params
     * @param string $format: NULL, or json,xml,serialize,php,csv
     */
    public function post_with_file($uri, $params = array(), $format = NULL) {
        $this->option(CURLOPT_BINARYTRANSFER, true);
        $this->option(CURLINFO_HEADER_OUT, true);
        $response = $this->_call('post_with_file', $uri, $params, $format);
        $request_header = $this->_curl->get_request_header();
        $this->log("HTTP POST Header:\n" . print_r($request_header, true), 'trace');
        $this->log("HTTP POST Parameters:\n" . print_r($params, true), 'trace');
        return $response;
        //return $this->_call('post_with_file', $uri, $params, $format);
    }
