...
class RelationWidget extends CWidget {
    ...
    public function renderBelongsToSelection() {
        if (strcasecmp($this->style, "dropDownList") == 0)
            ...
            echo CHtml::ActiveCheckBoxList(
                    $this->_model, $this->field, $this->getRelatedData(), $this->htmlOptions);
        else if (strcasecmp($this->style, "text") == 0) {
            $_relatedData = $this->getRelatedData();
            $_modelData = $this->getModelData($this->_model, $this->field);
            if(isset($_relatedData[$_modelData])) {
                echo $_relatedData[$_modelData];
            }
            else
                echo $_modelData;
        }
...
");
        ...
        echo CHtml::dropDownList($this->getListBoxName(), 0, CHtml::listData(
                        array_merge(
                                array('0' => $this->allowEmpty), $relatedmodels), $this->relatedPk, $this->fields), array(
            'options' => $options,
                )
        );
        echo '<ul id="selected">';
    ...
    protected function renderAddButton() {
        ...
        if (!$this->useLinkButton) {
            echo CHtml::Link(
                    is_string($this->showAddButton) ? $this->showAddButton : $string, $link);
        } else {
            echo CHtml::LinkButton(
                    is_string($this->showAddButton) ? $this->showAddButton : $string, array('submit' => $link));
        }
    }
