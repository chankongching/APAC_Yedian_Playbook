...
class RightsFilter extends CFilter {
    ...
    protected function preFilter($filterChain) {
        ...
        // Check if the controller.* should be allowed
        if (!empty($this->_allowedActions) && in_array(ucfirst($controller->id) . '.*', $this->_allowedActions) === true) {
            return true;
        }
        // Check if the controller.action should be allowed
        if (!empty($this->_allowedActions) && in_array(ucfirst($controller->id) . '.' . ucfirst($action->id), $this->_allowedActions) === true) {
            return true;
        }
