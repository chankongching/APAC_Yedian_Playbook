...
return array(
    ...
    'A role is group of permissions to perform a variety of tasks and operations, for example the authenticated user.' => '角色由一组权限组成，能够执行不同的任务和操作。',
    'Roles exist at the top of the authorization hierarchy and can therefore inherit from other roles, tasks and/or operations.' => '角色能够从其他角色，任务或者操作继承权限。',
    'A task is a permission to perform multiple operations, for example accessing a group of controller action.' => '任务是一种可以执行多项操作的权限，如执行一个模块的一组操作。',
    'Tasks exist below roles in the authorization hierarchy and can therefore only inherit from other tasks and/or operations.' => '任务可以从其他任务或者操作继承权限。',
    'An operation is a permission to perform a single operation, for example accessing a certain controller action.' => '操作是执行单项操作的权限，如访问模块的某项操作。',
    'Operations exist below tasks in the authorization hierarchy and can therefore only inherit from other operations.' => '除了自身权限设定外，操作只允许继承其他操作的权限。',
    'Generate items for controller actions' => '从系统模块中创建权限项目',
);
