...
/**
 * Description of CExGridExportBehavior
 *
 * @author SUNJOY
 */
 ...
 * //(Step 1) Add this behavior to the controller like
  ...
  'class' => 'application.components.exgrid.CExGridExportBehavior',
  ));
  }
 ...
 * @author Sun Joy <sunjoy99@163.com>
 * @link http://www.yiiframework.com/extension/exportablegridbehavior
 */
class CExGridExportBehavior extends CBehavior {
    ...
    public function renderExportGridButton($gridId, $label = 'Export', $buttonId = 'export-button', $htmlOptions = array()) {
        if (empty($buttonId)) {
        ...
        echo CHtml::link( '<i class="icon icon-arrowthick-s icon-white"></i>' . $label, '#', $htmlOptions);
        Yii::app()->clientScript->registerScript(
                'exportgrid' . $buttonId, "$('#" . $buttonId . "').on('click',function() { "
    ...
    public function createExportGridButton($label = 'Export', $buttonId = 'export-button', $htmlOptions = array()) {
        if (empty($buttonId)) {
            $buttonId = $this->buttonId;
        }
        if (!isset($htmlOptions['id'])) {
    ...
    public function registerExportGridButton(ExportGridView $grid, $buttonId = 'export-button') {
        if (empty($buttonId)) {
            $buttonId = $this->buttonId;
        }
        Yii::app()->clientScript->registerScript(
