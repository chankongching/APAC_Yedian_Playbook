...
class Xktv extends PSActiveRecord
{
    ...
    public function getKtvInfo($ktv, $_lat = 0, $_lng = 0)
    {
        ...
        $_xktv_info = array(
            'online_pay' => intval($ktv['online_pay']),
            'id' => intval($ktv['id']),
            "area_id" => $ktv['area_id'],
            "xktvid" => $ktv['xktvid'],
            "xktvname" => $ktv['name'],
...
//	public function getXktvList($_code, $_offset = 0, $_limit = 100) {
    ...
    public function getXktvList($_code, $_offset = 0, $_limit = 100, $_best = 'CONVERT(name USING GBK) asc', $type = '', $_ydzs = '0', $_sjq = '0', $_sjf = '0', $_taocan = '0', $_city = 440100)
    {
        $criteria = new CDbCriteria();
        if ($_best == 'rate asc') {
...
//		$_list = $this->findAllByAttributes(empty($_code) ? array() : array('area_id' => $_code), array('offset' => $_offset, 'limit' => $_limit, 'order' => $_best));
    ...
    public function getXktvcoodsList($city='')
    {
        $_list = $this->findAllByAttributes(array('city'=>$city));
        if (!empty($_list) && is_array($_list)) {
    ...
    public function searchXktvList($_name = '', $_address = '', $_telephone = '', $_responsetime = 0, $_rate = 0, $_offset = 0, $_limit = 100, $_xktvid = '', $city = '')
    {
//        echo $city;die();
        $criteria = new CDbCriteria;
