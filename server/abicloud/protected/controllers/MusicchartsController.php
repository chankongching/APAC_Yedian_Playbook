...
class MusicchartsController extends Controller
{
	/**
        ...
        public function behaviors() {
            return array(
                'exportableGrid' => array(
                    ...
                    'filename' => 'Charts-' . date('YmdHis', time()) . '.csv',
            ));
        }
	...
	{
		$model=new Musiccharts;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['Musiccharts']))
		{
			$model->attributes=$_POST['Musiccharts'];
                        ...
                        if ($model->validate()) {
                            if ($model->bpic_file !== null) {
                                ...
                                // 存储文件名  
                                $file_name = 'CHART_BPIC_' . date('YmdHis') . '_' . rand(1000, 9999) . '.' . $file_type;
                            ...
                            if ($model->spic_file !== null) {
                                ...
                                // 存储文件名  
                                $file_name = 'CHART_SPIC_' . date('YmdHis') . '_' . rand(1000, 9999) . '.' . $file_type;
			...
			if($model->save()) {
                                Yii::app()->user->setFlash('create', Yii::t('Musiccharts', '{name} created successfully.', array('{name}' => $model->name)));
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Musiccharts']))
		{
			...
			if($model->save()) {
                                Yii::app()->user->setFlash('update', Yii::t('Musiccharts', '{name} updated successfully.', array('{name}' => $model->name)));
				//$this->redirect(array('view','id'=>$model->id));
                                $this->redirect(array('index'));
                        }
		}
	...
	{
		$model=new Musiccharts('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Musiccharts']))
			$model->attributes=$_GET['Musiccharts'];
	 ...
	 * @return Musiccharts the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=Musiccharts::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param Musiccharts $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='musiccharts-form')
		{
        ...
        public function actionImport() {
            ...
            if(isset($_POST['Musiccharts'])) {
                        // read from file
                        $txtFile = CUploadedFile::getInstance($model, 'txtFile');
                        if ($txtFile !== null) {
