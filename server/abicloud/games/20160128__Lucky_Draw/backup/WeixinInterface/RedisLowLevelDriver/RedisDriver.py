...
from protos.redis import redis_pb2
DEFAULT_SERVER = '127.0.0.1:6379'
DEFAULT_SERVER_PARAMS = {'db':5 ,'timeout': None}

class DataStructError(Exception):
    pass

class RedisCache(BaseRedisCache):
    """
    single version for heng.lu design.
    all struct use use protobuf serial / deserial
    refactor_area
    """
    def serialize(self, value):
        """
        serialize by google protobuf
        """
        #return pickle.dumps(value, self.pickle_version)
        if type(value) == redis_pb2.RedisCache:
            return value.SerializeToString()
            pass
        else:
            raise DataStructError('only support proto struct union type')
    ...
    def deserialize(self, value):
        ...
        try:
            value = redis_pb2.RedisCache.ParseFromString(value)
            return value
        except Exception as err:
            logger.error('err when deserialize, msg: %s' % err.message)



    def __init__(self, server='127.0.0.1:6379', params={'db':5}):
        """
