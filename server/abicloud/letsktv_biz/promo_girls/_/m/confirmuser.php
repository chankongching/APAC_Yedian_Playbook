...
if(METHOD == 'GET') {
    ...
    // 获取数据源信息
    $query = sprintf("select `id`, `name`, `phone`, `status`, `assistant` from `%s%s` where `phone`='%s' limit 1;", 
        $C['db']['pfix'],
        'userdata',
    ...
    // 更新用户信息
    $query = sprintf("update `%s%s` set `status`=1, `datetime_bind`='%s', `assistant` = '%s' where `openid`='%s' limit 1;", 
        $C['db']['pfix'],
        'users',
        DATETIME, 
        $DB->real_escape_string($userdata['assistant']), 
        $DB->real_escape_string($_SESSION['letsktv_biz_promogirls_openid'])
    );
    $source = $DB->query($query);
    $DB->errno > 0 && die(json_encode(array(
