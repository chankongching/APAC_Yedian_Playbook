...
//为了没有appid的订阅号。
	...
	// 批量获取用户信息
	public function getUserInfoList($openids = array()) {
		if (!$this->access_token && !$this->checkAuth()) {
			return false;
		}
		$data = array('user_list' => $openids);
		$result = $this->http_post(self::API_URL_PREFIX . '/user/info/batchget?' . 'access_token=' . $this->access_token, self::json_encode($data));
		if ($result) {
...
// 		if ($appid == 'wx90f8e48d4b4f5d8d') {
		...
		// 			//				return $rs;
		// 			//			}
		// 			$result = $this->http_get('https://app.jingsocial.com/api/wechat/accessToken?appid=' . $appid . '&secret=' . $appsecret);
		// 			if ($result) {
		// 				$json = json_decode($result, true);
	...
	public function checkJingAuth($appid = '', $appsecret = '', $token = '') {
		if (!$appid || !$appsecret) {
			$appid = $this->appid;
			$appsecret = $this->appsecret;
		}
