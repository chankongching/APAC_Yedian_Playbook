...
class CouponController extends CommonController {
	...
	public function getShareCouponByConfirmOrder($userid = 0, $orderid = 0) {
		if ($userid != 0 && $orderid != 0) {
			...
			$result = M('CouponShare', 'ac_')->add(array('userid' => $userid, 'orderid' => $orderid, 'hash_url' => $hash_key, 'create_time' => date('Y-m-d H:i:s'), 'create_user_id' => session('openid'), 'expire_time' => time() + 7 * 24 * 60 * 60));
			if ($result > 0) {
				if ($this->getShareCouponHashKey($hash_key, $result, $orderid)) {
					return array('result' => 0, 'share_coupon_id' => $result, 'msg' => 'get success!', 'hash_url' => $hash_key);
				} else {
