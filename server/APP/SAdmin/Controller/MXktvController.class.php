...
class MXktvController extends CommonController {
	public function import($file = '') {
		ini_set('max_execution_time', '1000');
		if ($file == '') {
			// $filepath = 'excel/Beijing/Beijing.xlsx';
			// $filepath = 'excel/Shanghai/Shanghai.xlsx';
			// $filepath = 'excel/Guangdong/Foshan.xlsx';
			// $filepath = 'excel/Guangdong/Dongguan.xlsx';
			// $filepath = 'excel/Chengdu/chengdu.xlsx';
		...
		$sheettotal = $PHPExcel->getAllSheets();
		// var_dump($sheettotal);die();
		$erp_orders_ids = array();
		foreach ($sheettotal as $key => $value) {
...
//                        echo 'sfsdfsfsfsdfs';
	...
	public function dnew() {
		...
		$xx = $this->import();
		foreach ($xx as $kk => $vv) {
			foreach ($xx[$kk] as $key => $value) {
				if ($key != 'area' && $key != 2 && $value['name'] != '') {
					$ddd = $this->delspace($value);
					$ddd['city'] = 441900;
					$new_id = M('xktv', 'ac_')->add($ddd);
					if ($new_id > 0) {
						$xktvid = 'XKTV' . sprintf("%05d", $new_id);
						M('xktv', 'ac_')->where(array('id' => $new_id))->save(array('xktvid' => $xktvid));
						echo $value['name'] . '添加成功---';
					}
				}
			}
		}
	...
	private function delspace($vv = '') {
		foreach ($vv as $key => $value) {
			$vv[$key] = trim($value);
			$vv[$key] = $vv[$key] == '有' ? 1 : $vv[$key];
			$vv[$key] = $vv[$key] == '无' ? 0 : $vv[$key];
		}
		return $vv;
	}
	...
	public function tt() {
		...
		foreach ($xx as $kk => $vv) {
			...
			foreach ($xx[$kk] as $key => $value) {
				...
				if ($key != 'area' && $key != 2 && $key != 3 && $key != 4 && $value['name'] != '') {
					$ddd = $this->delspace($value);
					var_dump($ddd);
					$ddd['city'] = 440600;
					$new_id = M('xktv', 'ac_')->add($ddd);
